...
from monai.transforms import EnsureChannelFirst, Compose, ToTensor
...
def main():
    ...
    test_fpaths = [os.path.join(home_dir, exp_name, "test", filename.replace("nii.gz", "tiff")) for filename in
                   df_test['filename']]
    ...
    test_ds = ImageDataset(image_files=test_fpaths, labels=test_class_label, transform=transforms, image_only=True,
                           reader="PILReader")

    # create a validation data loader
    test_loader = DataLoader(test_ds, batch_size=BATCH_SIZE, num_workers=4, generator=g,
