...
def main(exp_name):
    ...
    data_path = Path(exps.path) / "pca"
    pca_path = data_path / f"train_{exps.dim_red}_{exps.n_comps}.pkl"

    if not os.path.exists(pca_path):
        raise RuntimeError(
            "Encoding does not exist. Please run the encoding script first."
        )
...
def train_macaw(exps, g):
    ...
    # Build causal graph
    disease_to_latents = [
        (0, i) for i in range(exps.n_causes, exps.n_causes + exps.n_evecs)
    ]
    bias_to_latents = [
        (1, i) for i in range(exps.n_causes, exps.n_causes + exps.n_evecs)
    ]
    autoregressive_latents = [
        (i, j)
        for i in range(exps.n_causes, exps.n_evecs + exps.n_causes)
        for j in range(i + 1, exps.n_evecs + exps.n_causes)
    ]
    ...
    priors = [
        (
            slice(0, 1),
            td.Bernoulli(torch.tensor([P_dis]).to(torch.device(exps.device))),
        ),  # disease
        (
            slice(1, 2),
            td.Bernoulli(torch.tensor([P_bias]).to(torch.device(exps.device))),
        ),  # bias
        (
            slice(exps.n_causes, exps.n_evecs + exps.n_causes),  # nevecs
            td.Normal(
                torch.zeros(exps.n_evecs).to(exps.device),
                torch.ones(exps.n_evecs).to(exps.device),
            ),
        ),
    ]
    ...
    for e in range(0, exps.n_comps - 1, exps.n_evecs):
        ...
        loss_vals_train, loss_vals_val = macaw.fit_with_priors(
            train_ds, val_ds, edges, priors, writer
        )
